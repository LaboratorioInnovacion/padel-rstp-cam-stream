...
if (isDev) {
  ...
  // Producción: usar carpeta de recursos extra
  ...
  const defaultCamerasFile = path.join(process.resourcesPath, 'cameras.json')
  const defaultConfigFile = path.join(process.resourcesPath, 'config.json')
  const defaultCloudflaredConfig = path.join(process.resourcesPath, 'cloudflared-config.yml')
  
  if (!fs.existsSync(path.join(dataDir, 'cameras.json')) && fs.existsSync(defaultCamerasFile)) {
    fs.copyFileSync(defaultCamerasFile, path.join(dataDir, 'cameras.json'))
    console.log('📋 cameras.json copiado a userData')
  }
  if (!fs.existsSync(path.join(dataDir, 'config.json')) && fs.existsSync(defaultConfigFile)) {
    fs.copyFileSync(defaultConfigFile, path.join(dataDir, 'config.json'))
    console.log('⚙️ config.json copiado a userData')
  }
  if (!fs.existsSync(path.join(dataDir, 'cloudflared-config.yml')) && fs.existsSync(defaultCloudflaredConfig)) {
    fs.copyFileSync(defaultCloudflaredConfig, path.join(dataDir, 'cloudflared-config.yml'))
    console.log('☁️ cloudflared-config.yml copiado a userData')
  }
}

console.log('🔍 Modo:', isDev ? 'DESARROLLO' : 'PRODUCCIÓN')
console.log('🔍 Binarios en:', binDir)
console.log('🔍 Datos en:', dataDir)

const camerasFile = path.join(dataDir, 'cameras.json')
const configFile = path.join(dataDir, 'config.json')
const mediamtxConfigFile = path.join(dataDir, 'mediamtx.yml')
...
function updateCloudflaredConfig() {
  ...
  console.log('📝 Actualizando cloudflared-config.yml...')
...
function loadConfig() {
  try {
    ...
    if (tunnelName) {
      ...
      // Si tiene tunnelId pero no existe cloudflared-config.yml, regenerarlo
      if (tunnelId) {
        const cloudflaredConfigPath = path.join(dataDir, 'cloudflared-config.yml')
        if (!fs.existsSync(cloudflaredConfigPath)) {
          console.log('🔧 Regenerando cloudflared-config.yml faltante...')
          updateCloudflaredConfig()
        } else {
          console.log(`✅ cloudflared-config.yml existe: ${cloudflaredConfigPath}`)
        }
        ...
        if (fs.existsSync(credentialsPath)) {
          console.log(`✅ Credenciales del túnel encontradas: ${credentialsPath}`)
        } else {
          console.warn(`⚠️ Credenciales del túnel NO encontradas en: ${credentialsPath}`)
          console.warn(`   Necesitarás ejecutar "cloudflared login" o reconfigurar el túnel`)
        }
      }
    }
...
function startProcess(name, args = [], options = {}, processKey = null) {
  ...
  console.log(`🚀 Intentando iniciar ${name}...`)
  console.log(`   Binario: ${bin(name)}`)
  console.log(`   Args: ${JSON.stringify(args)}`)
  console.log(`   Existe: ${fs.existsSync(bin(name))}`)
  
  if (!checkBinary(name)) {
    console.error(`❌ ${name} - binario no encontrado en ${bin(name)}`)
    lastError = `Binario ${name} no encontrado`
  ...
  console.log(`✅ Proceso ${name} iniciado con PID: ${p.pid}`)

  // logs en dataDir para que funcionen en producción
...
async function startMTX() {
  ...
  // MediaMTX con ruta absoluta al archivo de configuración
  const args = [mediamtxConfigFile]
  const options = {
    cwd: dataDir  // Establecer el directorio de trabajo
  }
  const proc = startProcess('mediamtx', args, options)
...
function startNamedTunnel() {
  ...
  // Con named tunnels, usar archivo de configuración desde dataDir
  const configPath = path.join(dataDir, 'cloudflared-config.yml')
  
  // Verificar que existe el archivo de configuración
  if (!fs.existsSync(configPath)) {
    console.error('❌ No se encuentra cloudflared-config.yml')
    console.error(`   Ruta esperada: ${configPath}`)
    console.error('   Crea el archivo o reconfigura el túnel')
    return null
  }
  
  // Verificar archivo de credenciales en directorio home del usuario
  const homeDir = require('os').homedir()
  const cloudflaredDir = path.join(homeDir, '.cloudflared')
  const credentialsFile = path.join(cloudflaredDir, `${tunnelId}.json`)
  
  console.log(`🔍 Verificando credenciales...`)
  console.log(`   Directorio: ${cloudflaredDir}`)
  console.log(`   Archivo: ${tunnelId}.json`)
  
  if (!fs.existsSync(credentialsFile)) {
    console.error('❌ No se encuentran las credenciales del túnel')
    console.error(`   Ruta esperada: ${credentialsFile}`)
    console.error('   💡 Ejecuta "cloudflared login" para autenticarte')
    console.error('   💡 O reconfigura el túnel desde el modal')
    lastError = 'Credenciales del túnel no encontradas'
    return null
  }
  
  console.log('✅ Archivo de credenciales encontrado')
  console.log(`✅ Archivo de configuración encontrado: ${configPath}`)
  ...
  p.stderr?.on('data', (data) => {
    ...
    if (output.includes('unable to find') && output.includes('credentials')) {
      console.error('❌ ERROR: No se pueden encontrar las credenciales del túnel')
      console.error(`   Buscar archivo: ${tunnelId}.json en ~/.cloudflared/`)
      console.error('   💡 Reconfigura el túnel desde el modal de configuración')
      lastError = 'Credenciales del túnel inválidas o no encontradas'
    }
    
    if (output.includes('context deadline exceeded') || output.includes('connection refused')) {
      console.error('❌ ERROR: No se puede conectar a Cloudflare')
      console.error('   Verifica tu conexión a Internet')
      lastError = 'No se puede conectar a Cloudflare'
    }
    ...
    if (!connected && output.includes('Registered tunnel connection')) {
      ...
      lastError = null // Limpiar errores previos
...
function getTunnelStatus() {
  const isRunning = processes['cloudflared'] && !processes['cloudflared'].killed
  ...
  if (tunnelId) {
    const homeDir = require('os').homedir()
    credentialsPath = path.join(homeDir, '.cloudflared', `${tunnelId}.json`)
  ...
  return {
    isConfigured: !!(tunnelName && tunnelId),
    isRunning,
    hasCredentials,
    hasConfig,
    tunnelUrl,
